...
const FollowedMembersCircle = ({ follower, idx, userDetails,messageWith }) => {
    ...
    return (
        ...
        >
            <div onClick={handleMessage} className={`relative flex w-max rounded-tr-full rounded-tl-full rounded-br-full  mx-auto  cursor-pointer hover:bg-black/20 duration-500 transition-all active:scale-90 p-1 pb-2      justify-between items-center text-black ${follower?._id===messageWith ? 'bg-black/20 overflow-hidden' : 'bg-transparent'}`}>
