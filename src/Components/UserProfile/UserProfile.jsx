...
senderMail: user?.email,
    ...
    const handleChat = () => {
        navigate(`/dashboard/connected_with/message?userId1=${logedInUser?._id}&userId2=${userData?._id}`)
    }
    return (
        ...
        >
            ...
            <div className="ProfileSection flex flex-col items-center justify-center gap-3 bmiNumber  p-[20px]">
                ...
                <div className={`${logedInUser?._id === userData?._id ? "hidden" : "Followbuttons flex justify-center items-center gap-2"}`}>
                    <button
                        className={`${!checkingFollower?.length > 0 ? "hidden" : "bg-secondary text-white p-[10px] rounded-full text-xl"} cursor-default`}>Follower</button>
                    <button
                        onClick={handleFollow}
                        className={`${checkingFollowing?.length > 0 ? "hidden" : "bg-secondary text-white p-[10px] rounded-full text-xl"} ${checkingFollower?.length > 0 ? "hidden" : "bg-secondary text-white p-[10px] rounded-full text-xl"}`}>Follow</button>
                    <button
                        onClick={unfollow}
